...
// import React, { useState } from "react";
// import GlobeImage from "../../assets/Globe_image.png";
 
// export default function Sidebar({ brand = "PaySphere", items = [], active, onChange, darkMode = true }) {
//   const [hovered, setHovered] = useState(null);
 
//   const bg     = darkMode ? 'linear-gradient(180deg, #1a1f2e 0%, #0f1419 100%)' : 'linear-gradient(180deg, #ffffff 0%, #f8fafc 100%)';
...
//   return (
//     <aside
//       className="h-screen w-64 flex flex-col shrink-0 transition-colors duration-300 sticky top-0"
//       style={{ background: bg, borderRight: `1px solid ${border}` }}
//     >
//       {/* Brand */}
//       <div className="flex items-center gap-3 px-5 h-16" style={{ borderBottom: `1px solid ${border}` }}>
//         <img src={GlobeImage} alt="PaySphere Logo" className="h-9.5 w-9.5 rounded-full object-cover scale-120" style={{ boxShadow: "0 0 25px rgba(16,185,129,0.55), inset 0 0 15px rgba(16,185,129,0.3)" }} />
//         <div>
//           <span className={`font-black tracking-tight text-base ${darkMode ? 'text-white' : 'text-slate-800'}`}>{brand}</span>
//           <p className="text-[9px] font-bold uppercase tracking-widest text-emerald-500/70 leading-none mt-0.5">Secure Payments</p>
//         </div>
//       </div>
//       <nav className="mt-3 px-3 space-y-0.5 overflow-y-auto flex-1">
//         {items.map((item) => {
...
export default function Sidebar({ brand = "PaySphere", items = [], active, onChange, darkMode = true }) {
  ...
  return (
    <aside
      // CHANGED: Added dynamic width (w-20 or w-64) and transition-all for smooth collapsing
      className={`h-screen flex flex-col shrink-0 transition-all duration-300 sticky top-0 ${isCollapsed ? 'w-20' : 'w-64'}`}
      style={{ background: bg, borderRight: `1px solid ${border}` }}
    >
      {/* Brand Header & Hamburger */}
      <div
        className={`flex items-center h-16 transition-all duration-300 ${isCollapsed ? 'justify-center' : 'justify-between px-4'}`}
      ...
      >
        ...
        {!isCollapsed && (
          <div className="flex items-center gap-3 overflow-hidden">
            <img
              ...
              style={{ boxShadow: "0 0 25px rgba(16,185,129,0.55), inset 0 0 15px rgba(16,185,129,0.3)" }}
            />
            <div className="whitespace-nowrap">
              <span className={`font-black tracking-tight text-base block leading-none mb-1 ${darkMode ? 'text-white' : 'text-slate-800'}`}>
                {brand}
              </span>
              <p className="text-[9px] font-bold uppercase tracking-widest text-emerald-500/70 leading-none">
                Secure Payments
              </p>
            </div>
          </div>
        ...
        <button
          ...
          className={`p-1.5 rounded-lg transition-colors flex items-center justify-center shrink-0
            ${darkMode ? 'text-slate-400 hover:text-white hover:bg-white/10' : 'text-slate-500 hover:text-slate-900 hover:bg-black/5'}
          `}
      ...
      {/* Nav */}
      <nav className={`mt-3 px-3 space-y-0.5 overflow-y-auto flex-1 ${isCollapsed ? 'flex flex-col items-center' : ''}`}>
        {items.map((item) => {
          const isActive = item.key === active;
          const isHovered = hovered === item.key;
          const highlight = isActive || isHovered;
          ...
          return (
            <button
              ...
              className={`group relative flex items-center py-2.5 rounded-xl text-left transition-all duration-200
                ${isCollapsed ? 'justify-center w-12 px-0' : 'w-full gap-3 px-3'}
              `}
              style={{
                background: isActive
                  ? (darkMode ? 'linear-gradient(135deg,rgba(16,185,129,0.18),rgba(5,150,105,0.08))' : 'linear-gradient(135deg,rgba(16,185,129,0.12),rgba(5,150,105,0.06))')
            ...
            >
              ...
              {/* Icon */}
              <span className="text-base flex justify-center items-center w-5 transition-all duration-200"
                style={{ filter: highlight && !isLogout ? "drop-shadow(0 0 6px #10b981)" : "none",
                  opacity: isLogout && !highlight ? 0.5 : 1 }}>
                {item.icon ?? <DotIcon />}
              </span>
              ...
              {!isCollapsed && (
                <span className="font-semibold text-sm transition-colors duration-200 whitespace-nowrap"
                  style={{ color: labelColor(highlight, isLogout) }}>
                  {item.label}
                </span>
              )}
 
              {/* Active Dot indicator on the right - Hidden when collapsed */}
              {!isCollapsed && isActive && (
                <span className="ml-auto w-1.5 h-1.5 shrink-0 rounded-full bg-emerald-400"
                  style={{ boxShadow: "0 0 6px #10b981" }} />
              )}
            </button>
          );
      ...
      {/* Footer */}
      <div className={`py-4 ${isCollapsed ? 'px-2 flex justify-center' : 'px-5'}`} style={{ borderTop: `1px solid ${border}` }}>
        {isCollapsed ? (
          <p className={`text-[10px] font-black ${darkMode ? 'text-white/15' : 'text-slate-400'}`}>©</p>
        ) : (
          <p className={`text-[9px] font-black uppercase tracking-widest whitespace-nowrap ${darkMode ? 'text-white/15' : 'text-slate-400'}`}>
            © {new Date().getFullYear()} PaySphere
          </p>
        )}
      </div>
    </aside>
