...
//         const response = await fetch("http://localhost:5245/api/users/me", {
...
export default function DashboardShell({
  ...
  activeTab,
  children,
  isLoading = false, // Added so parent components can trigger the loading screen
}) {
  const [active, setActive] = useState(defaultKey);
  const [isFetchingApi, setIsFetchingApi] = useState(false); // Tracks the user fetch
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        setIsFetchingApi(true); // Show loading screen
        ...
        setIsFetchingApi(false); // Hide loading screen
      }
    };
  ...
  const handleChange = (key) => {
    ...
    if (key !== currentTab) {
      setActive(key); // Tab changes instantly now
      if (onTabChange) onTabChange(key);
    }
  };

  // Only show the transition overlay if we are fetching user data OR if a parent component passed isLoading={true}
  const showOverlay = isFetchingApi || isLoading;

  return (
    <div className={`flex h-screen overflow-hidden ${theme.text} transition-colors duration-100`} style={{ background: theme.shell }}>
      <PageTransition isTransitioning={showOverlay} />
      <Sidebar brand={brand} items={navItems} active={currentTab} onChange={handleChange} darkMode={darkMode} />
